...
function initializeGeminiSearch() {
  ...
  // Single, efficient Ctrl+G listener using capture phase to intercept before Chrome's default handlers
  // Capture phase ensures we get the event before it bubbles down to other handlers
  document.addEventListener('keydown', async (e) => {
    // Check for Ctrl+G (without Shift)
    if (e.ctrlKey && !e.shiftKey && (e.key === 'G' || e.key === 'g')) {
      console.log('🔵 Ctrl+G detected - preventing default and handling request');
      
      // Prevent Chrome's default "Find Next" behavior
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      
      // Handle the AI request (protected by isProcessingRequest flag in handleAIRequest)
      try {
  ...
  }, true); // TRUE = capture phase (intercepts before Chrome's handlers)
  
  console.log('🔵 Gemini Search event listener initialized successfully!');
}

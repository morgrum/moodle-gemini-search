...
let dragEventListeners = { mousemove: null, mouseup: null };
...
function displayResponse(response, model) {
  ...
  popup.innerHTML = `
    ...
    <div style="background: rgba(255,255,255,0.1); padding: 15px; border-radius: 5px; white-space: pre-wrap;">${escapeHtml(response)}</div>
  ...
  // Store event listener references so they can be removed later
  ...
  dragEventListeners.mousemove = mousemoveHandler;
  dragEventListeners.mouseup = mouseupHandler;
  
  // Cleanup listeners when popup is removed via close button
  const closeButton = popup.querySelector('button');
  if (closeButton) {
    closeButton.addEventListener('click', () => {
      cleanupPopup();

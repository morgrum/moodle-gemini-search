...
// Promise queue to ensure atomic increment operations
let usageIncrementQueue = Promise.resolve();
...
// Function to atomically check availability and reserve a model slot
async function reserveModelSlot() {
  // Use promise queue to ensure atomic check-and-reserve
  usageIncrementQueue = usageIncrementQueue.then(async () => {
    ...
    // Atomically reserve the slot by incrementing usage
    const today = new Date().toDateString();
    const modelKey = `${selectedModel.name}_${today}`;
    usage[modelKey] = (usage[modelKey] || 0) + 1;
    await saveModelUsage(usage);
    
    return selectedModel;
  }).catch(error => {
    console.error('Error in reserveModelSlot:', error);
    throw error;
  });
  
  return usageIncrementQueue;
}
...
async function handleContentAnalysis(data) {
  ...
  try {
    ...
    // Atomically check availability and reserve a model slot
    // This prevents race conditions where multiple requests check and increment simultaneously
    const selectedModel = await reserveModelSlot();
...
Please provide a concise, helpful analysis of this content.`;
    ...
    // Usage was already incremented atomically when reserving the model slot

...
// SECURITY WARNING: Embedded API key for convenience
// ?? This is a security vulnerability - API keys should never be hardcoded in client-side code.
// In production, use one of the following approaches:
// 1. Store the API key in Chrome storage with encryption (still vulnerable to extension inspection)
// 2. Use a backend proxy server to make API calls (recommended for production)
// 3. Use Chrome Identity API with OAuth flow for secure key management
// 
// For now, this key is embedded for ease of use as mentioned in the README,
// but users should be aware that anyone can extract this key from the extension.
